...
$(function() {
	...
	var $tblRecommendationRunning = $("#tbl-recommendationRunning tbody");
	var $pgtRecommendationRunning = $("#pgt-recommendationRunning");
	...
	var endDate = Date.parse(dateSearch.split("-")[1].trim()).toString(
			'yyyy-MM-dd');
	searchRecommendationRunning(startDate, endDate, $txtSearch.val(), 1);
	$txtSearch.keyup($.debounce(1000, function() {
		searchRecommendationRunning(startDate, endDate, $txtSearch.val(), 1);
	}));

	function searchRecommendationRunning(startDate, endDate, keyword, pageNum) {
		$loading.LoadingOverlay("show");
		$.ajax({
			url : baseUrl + "/portal/recommendation/running/search",
			type : 'GET',
			dataType : "json",
			cache : false,
			data : {
			...
			success : function(res) {
				...
				$tblRecommendationRunning.html(html);
				var numOfPage = Math.ceil(res.recommendationCount / 5.0);
				console.log(numOfPage);
				$pgtRecommendationRunning.html(buildHTMLPagination(numOfPage,
						pageNum));
				$pgtRecommendationRunning.find("a")
						.click(
								function() {
									searchRecommendationRunning(startDate,
	...
	function buildHTMLResulTableRow(recommendation) {
		return "<tr class='even pointer'>"
				...
				+ recommendation.recommendationId
				+ "'"
				+ "class='btn-view btn btn-success btn-xs' data-toggle='modal' data-target='.bs-example-modal-lg' type='button'><i class='fa fa-eye fa-lg' "
				+ "style=' margin-left: 10px; margin-right: 10px; text-align: center;'></i></button></td>"
				+ "</tr>";
	}
	...
	$("#reportrange_right").on(
			...
			function(a, b) {
				...
				searchRecommendationRunning(startDate, endDate, $txtSearch
						.val(), 1);
			});
	...
	function buttonViewClick() {
		$('button.btn-view').click(function() {
			var $this = $(this);
			viewRecommendation($this.attr('data-rid'));
