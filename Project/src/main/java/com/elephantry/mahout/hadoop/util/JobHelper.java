...
public class JobHelper {
	...
	public static void copyLocalJarsToHdfs(String localJarsDir, String hdfsJarsDir, Configuration configuration)
			throws IOException {
		...
		if (jarFiles.isEmpty()) {
			...
			logger.info("Copying {} JAR files from local dir ({}) to HDFS dir ({} at {})",
					new Object[] { jarFiles.size(), localJarsDir, hdfsJarsDir, resolveHdfsAddress(configuration) });
	...
	public static void copyLocalJarsToHdfs(String localJarsDir, String hdfsJarsDir, List<String> fileNames, Configuration configuration)
			throws IOException {
		checkRequiredArgument(localJarsDir, "Local JARs dir is null");
		checkRequiredArgument(hdfsJarsDir, "HDFS JARs dir is null");
		checkRequiredArgument(configuration, "Configuration is null");

		Set<File> jarFiles = collectJarFilesFromLocalDir(localJarsDir, fileNames);
	...
	private static Set<File> collectJarFilesFromLocalDir(String localJarsDirPath, List<String> fileNames) {
		File directoryFile = new File(localJarsDirPath);
		...
		for (File libFile : directoryFile.listFiles()) {
			if (libFile.exists() && !libFile.isDirectory() && libFile.getName().endsWith(".jar")) {
				if (fileNames.contains(libFile.getName())) {
					jarFiles.add(libFile);
	...
	private static Set<Path> collectJarPathsOnHdfs(String hdfsJarsDir, Configuration configuration) throws IOException {
		...
		if (!fileSystem.exists(jarsDirPath)) {
			throw new IllegalArgumentException("Directory '" + hdfsJarsDir + "' doesn't exist on HDFS ("
					+ resolveHdfsAddress(configuration) + ")");
		}
		if (fileSystem.isFile(jarsDirPath)) {
			throw new IllegalArgumentException("Path '" + hdfsJarsDir + "' on HDFS ("
					+ resolveHdfsAddress(configuration) + ") is file, not directory");
		}
